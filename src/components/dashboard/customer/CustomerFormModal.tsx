...
interface CustomerFormModalProps {
  ...
  isOpen: boolean
  onClose: () => void
  // Changed onSubmit to accept FormData
...
export default function CustomerFormModal({ mode, isOpen, onClose, onSubmit, data }: CustomerFormModalProps) {
  ...
  const handleImageChange = (file: File | null, previewUrl: string | null) => {
    ...
    handleInputChange("profilePicturePreview", previewUrl) // Keep for client-side preview
  }
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    for (const key in formData) {
      if (formData.hasOwnProperty(key)) {
        // Special handling for File object
        if (key === "profilePictureFile") {
          if (formData[key] instanceof File) {
            submitFormData.append(key, formData[key])
          }
        } else if (key !== "profilePicturePreview") {
          // Exclude preview URL from submission
          submitFormData.append(key, formData[key])
        }
      }
    }

    // Explicitly add profilePictureUrl if it exists and no new file was uploaded
    // This ensures that if a user doesn't change the image, the existing URL is sent.
    if (!formData.profilePictureFile && formData.profilePictureUrl) {
      submitFormData.append("profilePictureUrl", formData.profilePictureUrl)
    }

    onSubmit(submitFormData) // Pass FormData directly
  }
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg max-w-2xl w-full mx-4 md:mx-auto my-auto overflow-y-auto max-h-[90vh]">
        <div className="relative">
          <h2 className="text-xl font-bold mb-4">
            {mode === "create" ? "Ajouter un nouveau membre" : "Mettre à jour le membre"}
          </h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="col-span-2 mb-4">
            <ImageUpload
              value={formData?.profilePicturePreview || formData?.profilePictureUrl} // Use preview if available, otherwise existing URL
            ...
            <input
              ...
              value={
                formData?.membershipStartDate
                  ? new Date(formData.membershipStartDate).toISOString().split("T")[0]
            ...
            <input
              ...
              value={
                formData?.membershipEndDate
                  ? new Date(formData.membershipEndDate).toISOString().split("T")[0]
                  : new Date().toISOString().split("T")[0]
              }
